OSM.initializeMissingBuildingsDataLayer = function (map) {
  let dataLoader, loadedBounds;
  const dataLayer = map.missingBuildingsDataLayer;
  ...
  function getData() {
    const bounds = map.getBounds();
    const url = "/aero/api/" + OSM.API_VERSION + "/map.json?status=missing&bbox=" + bounds.toBBoxString();
