module Api
  class HistoryMapsController < ApiController
    ...
    def show
      ...
      begin
        ...
        raise OSM::APIBadUserInput, "The parameter year is required" unless params[:year]
      ...
      nodes = Node.joins(:ways => :way_tags).bbox(@bounds).where("current_way_tags.k = ? AND current_way_tags.v LIKE ?", "from_date",
                                                                 "#{params[:year]}%").where(:visible => true).includes(:node_tags).limit(Settings.max_number_of_nodes + 1)
    ...
    # def available_years
    #   @available_years ||= WayTag.where(k: "from_date").order(:v).distinct.pluck(:v)

    #   render :json => @available_years
    # end
  end
end
